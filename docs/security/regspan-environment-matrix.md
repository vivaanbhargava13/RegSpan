...
| `REGSPAN_EVAL_ENABLED` | Corpus evaluation runner | Server-only | `true` only for intentional production evaluation; also gates the development-only `--bypass-app-limits` profile, which remains unavailable in production | Restart not required |
