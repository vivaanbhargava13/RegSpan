...
function hasCustomerNoticeObligation(sentence) {
  return hasOperativeObligation(sentence)
...
function hasOperativeServiceProviderGovernance(sentence) {
  ...
  return /\b(?:firm|company|organization|institution|procurement|compliance)\b[^.!?]{0,100}\b(?:conducts?|performs?|maintains?|monitors?|oversees?|requires?)\b[^.!?]{0,140}\b(?:service[- ]providers?|vendors?|suppliers?|third[- ]part(?:y|ies))\b/.test(sentence);
}
...
function writtenComplianceRecordElementMatches(elementId, value) {
  ...
  switch (elementId) {
    ...
    case "retention_accessibility":
      return /\b(?:retain(?:s|ed|ing)?|retention period|six years|easily accessible place|accessible storage)\b/.test(normalized);
    default:
      return null;
  }
}
...
function externalNotificationCoordinationElementMatches(elementId, value) {
  const normalized = normalize(value);
  const incidentSpecificExternalNotification = /\b(?:incident|breach|security event|unauthorized access|cyber(?:security)? event)\b/.test(normalized)
  ...
  switch (elementId) {
    ...
    case "legal_compliance_coordination":
      return /\b(?:coordinat(?:e|es|ed|ion)|legal|compliance|privacy counsel|general counsel)\b/.test(normalized)
        && /\b(?:decide|decision|determine|determination|review|assess|evaluate|coordinate|notification|reporting)\b/.test(normalized);
    default:
      return null;
  }
}
