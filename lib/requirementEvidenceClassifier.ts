...
function sentenceContaining(rawText: string, signals: string[]) {
  ...
  return matched.length > 260 ? `${matched.slice(0, 257).trimEnd()}…` : matched;
}
  ...
  return {
    ...
    async classify(input) {
      ...
      try {
        ...
        const parsed = parseOpenAiClassification(await response.json());
        return {
          ...parsed,
