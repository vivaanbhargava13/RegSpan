...
export function Topbar({ session, workspaceName }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-app-border bg-app-shell/88 shadow-[0_1px_0_rgb(var(--app-border)/0.45)] backdrop-blur-xl">
      <div className="flex min-h-[72px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-10">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden items-center gap-3 border-r border-app-border pr-4 sm:flex">
            <span className="grid size-9 place-items-center rounded-full bg-app-accent-soft text-xs font-bold text-app-accent ring-1 ring-app-accent/10">
              {initials || "R"}
            </span>
      ...
      <nav className="flex gap-2 overflow-x-auto border-t border-app-border px-4 py-2 md:hidden" aria-label="Mobile navigation">
        {appNavLinks.map((link) => {
          ...
          return (
            <Link
              ...
              className={`whitespace-nowrap rounded-lg px-3 py-2 text-sm font-semibold outline-none transition-colors focus-visible:ring-4 focus-visible:ring-app-accent-soft ${
                isActive
